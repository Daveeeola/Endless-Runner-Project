...
{
	if(SmallObstacleClass && BigObstacleClass && CoinItemClass)
	{
		int32 NumBigs = 0;
...
void AFloorTile::SpawnLaneItem(UArrowComponent* Lane, int32& NumBigs)
{
	...
	if (UKismetMathLibrary::InRange_FloatFloat(RandVal,SpawnPercent1,SpawnPercent2,true,false))
	{
		AObstacle* Obstacle = GetWorld()->SpawnActor<AObstacle>(SmallObstacleClass, SpawnTransform, SpawnParams);
	...
	else if (UKismetMathLibrary::InRange_FloatFloat(RandVal,SpawnPercent2,SpawnPercent3,true,true))
	{
		if(NumBigs <= 1)
		{
			AObstacle* Obstacle = GetWorld()->SpawnActor<AObstacle>(BigObstacleClass, SpawnTransform, SpawnParams);
			if(Obstacle)
			{
				NumBigs += 1;
				ChildActors.Add(Obstacle);
	...
	else if (UKismetMathLibrary::InRange_FloatFloat(RandVal,SpawnPercent3,1.f,true,false))
	{
		ACoinItem* Coin = GetWorld()->SpawnActor<ACoinItem>(CoinItemClass, SpawnTransform, SpawnParams);
