...
void UGameHud::InitializeHUD(AEndlessRunnerGameModeBase* RunnerGameMode)
{
	if (RunnerGameMode)
	{
		CoinsCount->SetText(FText::AsNumber(0));

		LivesCount->SetText(FText::AsNumber(RunnerGameMode->MaxLives));

		RunnerGameMode->OnCoinsCountChanged.AddDynamic(this, &UGameHud::SetCoinSCount);
		RunnerGameMode->OnLivesCountChanged.AddDynamic(this, &UGameHud::SetLivesCount);
	}
}
